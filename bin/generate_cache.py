...
def main(_):
    ...
    train_set, train_samples = create_dataset(FLAGS.train_files.split(','),
                                              batch_size=1,
                                              enable_cache=True,
                                              cache_path=FLAGS.feature_cache)

    bar = create_progressbar(prefix='Preprocessing | ',
                             widgets=['Steps: ', progressbar.Counter(), ' | ', progressbar.Timer(), ' | ', progressbar.AdaptiveETA()],
                             max_value=train_samples).start()
