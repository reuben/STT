...
COPY --from=linux-cpu-opt-lib-build /code/native_client/libdeepspeech.so /
...
COPY --from=linux-cpu-opt-lib-build-artifact /libdeepspeech.so /code/tensorflow/bazel-bin/native_client/libdeepspeech.so
COPY . /code
...
COPY --from=linux-py36-cpu-opt /code/native_client/python/dist/deepspeech-*-cp36* /
...
COPY --from=linux-py37-cpu-opt /code/native_client/python/dist/deepspeech-*-cp37* /
...
COPY --from=linux-py38-cpu-opt /code/native_client/python/dist/deepspeech-*-cp38* /
...
COPY --from=linux-py39-cpu-opt /code/native_client/python/dist/deepspeech-*-cp39* /
