           ...
           bool extended_output, bool json_output)
{
  ...
  if (extended_output) {
    Metadata *result = DS_SpeechToTextWithMetadata(aCtx, aBuffer, aBufferSize, 1, key_word_spotter_mode, labels);
    res.string = CandidateTranscriptToString(&result->transcripts[0]);
    DS_FreeMetadata(result);
  } else if (json_output) {
    Metadata *result = DS_SpeechToTextWithMetadata(aCtx, aBuffer, aBufferSize, json_candidate_transcripts, key_word_spotter_mode, labels);
    res.string = MetadataToJSON(result);
    DS_FreeMetadata(result);
  } else if (stream_size > 0) {
    StreamingState* ctx;
    int status = DS_CreateStream(aCtx, &ctx, key_word_spotter_mode, labels);
    if (status != DS_ERR_OK) {
      res.string = strdup("");
      return res;
    }
    ...
    res.string = DS_SpeechToText(aCtx, aBuffer, aBufferSize, key_word_spotter_mode, labels);
  }
